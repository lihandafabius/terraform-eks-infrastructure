...
Using **Jenkins as a push-based GitOps model** also came with trade-offs worth noting. It required installing and maintaining an external toolchain (**Docker, Docker Compose, kubectl, Terraform**) directly on the Jenkins server, along with managing static AWS credentials for it — overhead a pull-based controller running natively inside the cluster, with its own in-cluster identity, wouldn't carry. 
